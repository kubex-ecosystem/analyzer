...
import (
	...
	mod "github.com/kubex-ecosystem/gemx/analyzer/internal/module"
)
...
func TestModule_Basics(t *testing.T) {
	m := mod.RegX()
	if !m.Active() {
		t.Fatalf("module should be active")
	}
	if m.Module() != "grompt" {
		t.Fatalf("Module() = %q, want 'grompt'", m.Module())
	}
	if m.ShortDescription() == "" || m.LongDescription() == "" {
		t.Fatalf("descriptions must not be empty")
	}
	if m.Usage() == "" {
