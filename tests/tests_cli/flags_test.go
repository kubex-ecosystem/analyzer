...
import (
	...
	cli "github.com/kubex-ecosystem/gemx/analyzer/cmd/cli"
...
func TestAskCommand_FlagsAndDefaults(t *testing.T) {
	...
	if ask == nil {
		t.Fatalf("ask command not found")
	}
	...
	for _, c := range cases {
		...
		if f.DefValue != c.def {
			t.Fatalf("flag %q default = %q, want %q", c.name, f.DefValue, c.def)
		}
	}
...
func TestGenerateCommand_FlagsAndDefaults(t *testing.T) {
	...
	if gen == nil {
		t.Fatalf("generate command not found")
	}
...
func TestStartServerCommand_FlagsAndDefaults(t *testing.T) {
	...
	for _, c := range cli.ServerCmdList() {
		if c.Use == "start" {
			start = c
			break
		}
	...
	if start == nil {
		t.Fatalf("start server command not found")
	}
