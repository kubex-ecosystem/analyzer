...
func printDaemonInfo(config daemon.DaemonConfig) {
	...
	gl.Log("info", "🚀 ============================================================")
	gl.Log("info", "")
	gl.Log("info", "🏗️  GoBE Integration: %s", config.GoBeURL)
	gl.Log("info", "📅 Auto Schedule: %v", config.AutoScheduleEnabled)
	if config.AutoScheduleEnabled {
		gl.Log("info", " (%s)", config.ScheduleCron)
	}
	gl.Log("info", "")
	gl.Log("info", "🔔 Notifications: %v", config.NotificationChannels)
	gl.Log("info", "🏥 Health Checks: every %v", config.HealthCheckInterval)
	gl.Log("info", "")
